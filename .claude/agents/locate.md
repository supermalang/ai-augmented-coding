...
Before doing anything, read `.claude/skills/locate/SKILL.md` and follow it **exactly** — it is your complete playbook. Then read `.claude/context.md`, the router index `.claude/code-map.md` if it exists (match the request to an area before grepping), and the **Code map (navigation)** section of `docs/ARCHITECTURE.md` for semantics if it exists.
