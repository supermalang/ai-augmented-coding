...
4. Read existing files at the paths listed under **Impact → Components** to understand patterns in place. If a **change-set** is available — either passed in the prompt or saved in the task's **Change-set (locate)** field by `/planner` — start from it: verify the targets, read the "read-for-context" files, and edit in the suggested order, instead of re-discovering the structure. A planning-time change-set is *coarse* (target areas), so sharpen it to the exact edits as you go. If none exists and the codebase is non-trivial, consider running `/locate` first.
