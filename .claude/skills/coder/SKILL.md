...
4. Read existing files at the paths listed under **Impact → Components** to understand patterns in place. If a **change-set from `/locate`** was provided (targets, call path, edit order, ripples), start from it — verify the targets, read the "read-for-context" files, and edit in the suggested order — instead of re-discovering the structure. If none was provided and the codebase is non-trivial, consider running `/locate` first.
