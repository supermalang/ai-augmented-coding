...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/diffs') {
    const diffs = findDiffs({ outputDir, baselinesDir }).map((d) => ({
      ...
      actual: '/img?p=' + encodeURIComponent(d.actual.replace(ROOT + '/', '').replace(ROOT + '\\', '')),
      diff: d.diff ? '/img?p=' + encodeURIComponent(d.diff.replace(ROOT + '/', '').replace(ROOT + '\\', '')) : null,
      _baseline: d.baseline, _actual: d.actual,
    }));
...
server.listen(PORT, () => {
  ...
  console.log(`  candidates: ${outputDir} (container-rendered)`);
