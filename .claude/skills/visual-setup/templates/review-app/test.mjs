...
// Run: node .claude/skills/visual-setup/templates/review-app/test.mjs
...
const baselinesDir = join(root, 'baselines', 'home.visual.spec.ts');
...
ok(diffs[0] && diffs[0].id === 'home.visual.spec.ts/home-desktop-linux.png', 'findDiffs: id is baseline-relative POSIX path');
...
approve({ id: diffs[0].id, task: 'VBR-5', actualPath: actual, baselinePath: baseline, approvalsFile, at: '2026-07-03T00:00:00Z' });
...
ok(appr[diffs[0].id]?.task === 'VBR-5', 'approve: records the task id');
ok(appr[diffs[0].id]?.capturedImage === actual, 'approve: records the container-rendered candidate (parity provenance)');

// 3 — reject records without re-baselining
writeFileSync(baseline, 'OLD-AGAIN');
reject({ id: diffs[0].id, task: 'VBR-5', approvalsFile, at: '2026-07-03T00:01:00Z' });
