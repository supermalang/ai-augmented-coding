...
- **`pr-reviewer`** is the primary agent that can `git push` / open PRs. The one other PR-capable agent is **`hill-climb`** (self-improvement, default-off) — but it is **propose-only**: `guard-hill-climb.sh` blocks it from writing anything except its proposal doc (`docs/improvements/**`), so it can open a PR of *suggestions* yet can never edit a skill/agent/hook/config or merge. Neither agent merges.
...
- **Models** are chosen by **tier**, not hardcoded per agent. Each envelope's `model:` names one of three tiers — `reasoning` · `standard` · `fast` — resolved to a concrete model in `.claude/context.md` → *Model tiers* (single source of truth; re-point a tier there to re-tier every agent in it). Tiers track **reasoning difficulty and compounding cost of error, not pipeline phase**: `reasoning` = `coder`/`debugger`/`schema-agent`/`hill-climb` (+`planner`); `standard` = the reviewers (`security-audit`/`performance`/`qa-tester`/`ux-review`/`dep-audit`) + `pr-reviewer`/`test-writer`/`setup`/`refactor`/`visual-setup`/`release`; `fast` = read-only reporters & template/trace work (`docs`/`report`/`diagram`/`locate`/`commit`/`code-map`/`visual-report`/`visual-review`). Per-agent concrete overrides are possible but must be justified by measured cost/latency, not guessed.
