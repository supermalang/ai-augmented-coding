...
| `release` | **Release management at the `develop → main` promotion.** Derives the semver bump from conventional commits since the last tag (`fix`→patch, `feat`→minor, `!`/`BREAKING CHANGE`→major), bumps the version file, updates `CHANGELOG.md`, drafts release notes (`docs/releases/<version>.md`), and creates a **local** tag (or a release PR). **Publishing is a separate `[CONFIGURE]`, default-off step** — unset → computes version + notes + local tag and publishes nothing; a human confirms any publish. Never changes the promotion gate; never auto-publishes |
| `hill-climb` | **Self-improvement (default-off, propose-only).** Reads the shipped run traces + `/retro` outputs, finds recurring pipeline patterns, and opens a **PR of evidence-linked harness suggestions** — it applies nothing and merges nothing (`guard-hill-climb` makes direct harness edits structurally impossible). Runs only in `propose-only` mode above `Min trace volume` (both in `.claude/context.md`). Read-only on the harness |
| `hotfix` | **Production-incident fast-lane.** Branches from the production branch (default `main`, not `develop`), reproduces the bug as a **failing regression test first**, applies the minimal fix, runs the **relevant** guards + a **focused** review, and opens a PR into production — **never auto-merging**. After merge: **back-merge production → `develop`** and **backfill** the incident into a roadmap `Type: Fix` task + run trace. Fast skips *ceremony*, never *safety*; `guard-hotfix-test` enforces the regression test |
...
- **`pr-reviewer`** is the primary agent that can `git push` / open PRs. Two others are PR-capable: **`hill-climb`** (self-improvement, default-off) — **propose-only**, `guard-hill-climb.sh` blocks it from writing anything except its proposal doc (`docs/improvements/**`), so it opens a PR of *suggestions* yet can never edit a skill/agent/hook/config; and **`hotfix`** (incident fast-lane) — opens a PR into the production branch with a mandatory regression test (`guard-hotfix-test.sh`). **None of the three merge** — the human merge gate is unchanged for all of them.
...
- **Models** are chosen by **tier**, not hardcoded per agent. Each envelope's `model:` names one of three tiers — `reasoning` · `standard` · `fast` — resolved to a concrete model in `.claude/context.md` → *Model tiers* (single source of truth; re-point a tier there to re-tier every agent in it). Tiers track **reasoning difficulty and compounding cost of error, not pipeline phase**: `reasoning` = `coder`/`debugger`/`schema-agent`/`hill-climb`/`hotfix` (+`planner`); `standard` = the reviewers (`security-audit`/`performance`/`qa-tester`/`ux-review`/`dep-audit`) + `pr-reviewer`/`test-writer`/`setup`/`refactor`/`visual-setup`/`release`; `fast` = read-only reporters & template/trace work (`docs`/`report`/`diagram`/`locate`/`commit`/`code-map`/`visual-report`/`visual-review`). Per-agent concrete overrides are possible but must be justified by measured cost/latency, not guessed.
...
| Bash / Edit / Write | `guard-hill-climb.sh` | The self-improvement (`hill-climb`) agent writing anything but its proposal — on a `hill-climb/*` run it denies every write outside `docs/improvements/**` (and `.scratch/**`), so direct edits to skills/agents/hooks/prompts/config are **structurally impossible**; propose-only, never merges. Inert on any non-hill-climb run. **pure-bash, fails closed** |
| Bash | `guard-hotfix-test.sh` | A push / PR-open from a `hotfix/*` branch that adds **no regression test** (vs the production branch) — a hotfix PR must include a test. Inert off hotfix branches and on non-push commands; a hotfix has **no** guard exemptions, this only *adds* the test requirement. |
