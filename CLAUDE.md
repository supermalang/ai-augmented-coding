...
- **Models** are chosen by **tier**, not hardcoded per agent. Each envelope's `model:` names one of three tiers — `reasoning` · `standard` · `fast` — resolved to a concrete model in `.claude/context.md` → *Model tiers* (single source of truth; re-point a tier there to re-tier every agent in it). Tiers track **reasoning difficulty and compounding cost of error, not pipeline phase**: `reasoning` = `coder`/`debugger`/`schema-agent`/`hill-climb` (+`planner`); `standard` = the reviewers (`security-audit`/`performance`/`qa-tester`/`ux-review`/`dep-audit`) + `pr-reviewer`/`test-writer`/`setup`/`refactor`/`visual-setup`; `fast` = read-only reporters & template/trace work (`docs`/`report`/`diagram`/`locate`/`commit`/`code-map`/`visual-report`/`visual-review`). Per-agent concrete overrides are possible but must be justified by measured cost/latency, not guessed.
